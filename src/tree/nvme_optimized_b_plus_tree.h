        ...
        static void *context_based_process(void* para) {
            ...
            while (!tree->working_thread_terminate_flag_) {
                ...
                if (tree->request_queue_.size() > 0 && tree->free_context_slots_ > 0) {
                    ...
                    tree->lock_.release();
                    tree->blk_accessor_->process_completion();
                    continue;
                }
            }
        }
    ...
    private:
        ...
        public:
            ...
            int run() {
                switch (this->status) {
                    case 0:
                        tree_->blk_accessor_->asynch_read(node_ref_->blk_address_, buffer_, this);
                    ...
                    case 1:
                        ...
                        switch (node_type) {
                            ...
                            case INNER_NODE:
                                ...
                                current_node_->deserialize(buffer_);
                                int child_index = dynamic_cast<InnerNode<K, V, CAPACITY>*>(current_node_)->locate_key(request_->key);
                                if (child_index < 0) {
                                    ...
                                    node_ref_->copy(dynamic_cast<InnerNode<K, V, CAPACITY>*>(current_node_)->child_[child_index]);
                                    delete current_node_;
                                    current_node_ = 0;
